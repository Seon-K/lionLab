...
function ListingForm({ mode = "create", initialValues, onSubmit }) {
  ...
  const [coverPreview, setCoverPreview] = useState(
    initialValues?.book?.cover_image ?? "",
  );
  const [initialBookId] = useState(
    () => initialValues?.book?.id ?? initialValues?.bookId ?? "",
  );
  ...
  const [form, setForm] = useState(() => ({
    ...
    courseName:
      initialValues?.course?.course_name ??
      initialValues?.course?.name ??
      initialValues?.courseName ??
      "",
    delete_password: initialValues?.delete_password ?? "",
    used_price: initialValues?.used_price ?? initialValues?.price ?? "",
    has_writing: String(
      initialValues?.has_writing ?? defaultValues.has_writing,
    ),
  }));
  ...
  useEffect(() => {
    Promise.all([getBooks(), getCourses()])
      .then(([bookData, courseData]) => {
        ...
        const currentBook =
          initialValues?.book ??
          bookData.find((book) => book.id === Number(initialBookId)) ??
          null;
        if (currentBook) {
  ...
  const previewBook = useMemo(() => {
    if (selectedBook)
      return {
        ...selectedBook,
        ...bookDraft,
        cover_image: coverPreview || bookDraft.cover_image,
      };
  ...
  const handleSearch = async (event) => {
    ...
    try {
      ...
      setSearchMessage(
        results.length
          ? `${results.length}개의 비슷한 교재를 찾았습니다.`
          : "검색 결과가 없습니다. 직접 입력해 주세요.",
      );
    } finally {
  ...
  const ensureBook = async () => {
    ...
    const existingBook = books.find(
      (book) =>
        (bookDraft.isbn && book.isbn === bookDraft.isbn) ||
        book.title === title,
    );
    ...
    <form className="listing-form enhanced-form" onSubmit={handleSubmit}>
      <section className="form-section book-register-section">
        ...
        <div className="book-search-field">
          <p className="search-guide-text">
            ISBN 또는 책 제목으로 검색한 뒤 교재를 선택하면 아래 교재 정보가
            자동으로 채워집니다.
          </p>
          <div className="inline-search">
            ...
            />
            <Button type="button" disabled={isSearching} onClick={handleSearch}>
              검색
            </Button>
          </div>
          ...
          {!isSearching && visibleSearchResults.length > 0 && (
            <div className="book-recommend-box">
              ...
              <div
                className="book-result-list"
                aria-label="교재 검색 결과 목록"
              >
                {visibleSearchResults.map((book) => (
                  ...
                  >
                    <img
                      src={book.cover_image || coverPlaceholder}
                      alt={`${book.title} 표지`}
                    />
                    <span>
                      <strong>{book.title}</strong>
                      <small>
                        {book.author || book.authors || "저자 정보 없음"}
                      </small>
        ...
        <div className="form-grid two-columns">
          <label>
            ...
            <input
              ...
              onChange={handleBookDraftChange}
              placeholder="예: 파이썬 알고리즘 인터뷰"
              required
            />
          </label>
          <label>
          ...
          <label>
            ...
            <input
              name="author"
              value={bookDraft.author || bookDraft.authors}
              onChange={handleBookDraftChange}
              placeholder="예: 박상길"
            />
          </label>
          ...
          <label>
            ...
            <input
              name="original_price"
              value={bookDraft.original_price}
              onChange={handleBookDraftChange}
              inputMode="numeric"
              placeholder="예: 38000"
            />
          ...
          <label className="wide-field">
            ...
            <input
              name="cover_image"
              value={bookDraft.cover_image || ""}
              onChange={handleBookDraftChange}
              placeholder="검색 이미지가 없으면 이미지 URL을 입력하세요."
            />
          </label>
        ...
        <div className="selected-book-preview">
          <img
            src={previewBook.cover_image || coverPlaceholder}
            alt={`${previewBook.title || "교재"} 표지`}
          />
          <div>
            <strong>
              {previewBook.title || "교재를 검색하거나 직접 입력하세요."}
            </strong>
            <p>
              {previewBook.author || previewBook.authors || "저자 정보"} ·{" "}
              {previewBook.publisher || "출판사 정보"}
            </p>
      ...
      <section className="form-section price-preview">
        ...
        <p>
          정가 대비 {originalDiscount}% 할인 · 현재 판매가 대비 {saleDiscount}%
          할인
        </p>
      </section>
